...
export default function TokenFactory() {
  ...
  const copyToClipboard = async (address: string) => {
    try {
      ...
      showToast("success", "복사 완료", "컨트랙트 주소가 클립보드에 복사되었습니다!");
    } catch (error) {
  ...
  const transferToken = async () => {
    if (!selectedToken || !transferTo || !transferAmount || !isWalletConnected) {
    ...
    try {
      ...
      showToast("info", "트랜잭션 전송", `트랜잭션이 블록체인에 전송되었습니다. 해시: ${tx.hash.slice(0, 10)}...`, 8000);

      await tx.wait();
      showToast("success", "전송 완료", `${transferAmount} ${selectedToken.symbol} 토큰이 성공적으로 전송되었습니다!`);
      ...
      showToast("error", "전송 실패", error.message || "알 수 없는 오류가 발생했습니다.");
    } finally {
  ...
  const loadTokensFromAPI = async (page: number = 1) => {
    try {
      ...
      const formattedTokens: DeployedToken[] = await Promise.all(
        tokensData.map(async (token: any) => {
          const balance = await getTokenBalance(token.address || "0x0");
          return {
  ...
  const deployToken = async () => {
    ...
    try {
      ...
      const initialSupplyWei = initialSupply;
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-start flex-1 min-w-0">
                  <div className="flex-shrink-0 mr-3">
                    ...
                    {toast.type === "info" && <Loader className="h-5 w-5 text-blue-400" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate">{toast.title}</p>
                    <p className="mt-1 text-sm text-gray-500 break-words">{toast.message}</p>
                  </div>
                </div>
                <button
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          {!isWalletConnected && (
            <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-center">
              <p className="text-yellow-800">토큰을 발행하려면 상단 우측의 &quot;지갑 연결&quot; 버튼을 클릭하세요</p>
            </div>
          )}
          ...
          <div className="flex flex-col lg:flex-row lg:items-end lg:space-x-4 space-y-4 lg:space-y-0">
            ...
            <div className="lg:w-40">
              <label className="block text-sm font-medium text-gray-700 mb-2">발행량</label>
              <input
                type="number"
                value={initialSupply}
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {deployedTokens.map((token, index) => {
                    // 내 소유 토큰 확인
                    const isMyToken = isWalletConnected && walletAddress.toLowerCase() === token.owner.toLowerCase();
                      ...
                      <tr key={index} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          {isWalletConnected ? (
                            ...
                            <div className="text-xs text-gray-400">지갑 연결 필요</div>
                          )}
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center space-x-2">
                            ...
                            {/* 전송 버튼 */}
                            {isWalletConnected && parseFloat(token.balance || "0") > 0 && (
                              <button
                                onClick={() => {
              ...
              <div className="flex items-center justify-between">
                <div className="text-sm text-gray-700">
                  ...
                  <span className="font-medium">
                    {(currentPage - 1) * TOKENS_PER_PAGE + 1}-{Math.min(currentPage * TOKENS_PER_PAGE, totalTokens)}
                  </span>
                  번째 표시
                </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">받는 주소</label>
                  <input
                    type="text"
                    value={transferTo}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">전송 수량</label>
                  <div className="flex items-center space-x-2">
                    <input
                      type="number"
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    ...
                    {parseFloat(selectedToken.balance || "0") > 0
                      ? parseFloat(selectedToken.balance || "0").toLocaleString("ko-KR", {
                          minimumFractionDigits: 0,
                          maximumFractionDigits: 6,
                        })
                      : "0"}{" "}
